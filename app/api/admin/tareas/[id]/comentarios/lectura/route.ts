...
export async function PUT(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const totalComentarios = ids.length;

    if (ids.length === 0) {
      return NextResponse.json(
        { ok: true, marked: 0, total_comentarios: 0, total_leidos_por_mi: 0, unread_before: 0, unread_after: 0 },
        { status: 200 }
      );
    }
    ...
    const unreadBefore = totalComentarios - leidosSet.size;
      ...
      return NextResponse.json(
        {
          ...
          total_leidos_por_mi: leidosSet.size,
          unread_before: unreadBefore,
          unread_after: unreadBefore,
        },
        { status: 200 }
      );
    }

    const { error: insErr } = await admin.from("tarea_comentario_lecturas").insert(nuevos);
    if (insErr) return NextResponse.json({ error: insErr.message }, { status: 500 });

    const totalLeidosPorMi = leidosSet.size + nuevos.length;
    const unreadAfter = totalComentarios - totalLeidosPorMi;

    return NextResponse.json(
      {
        ok: true,
        marked: nuevos.length,
        total_comentarios: totalComentarios,
        total_leidos_por_mi: totalLeidosPorMi,
        unread_before: unreadBefore,
