...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error: dbErr } = await admin.from("cursos").update({
      ...
      precio: Number(precio) || 0, image_id: image_id || "1", chat_url: chat_url.trim(),
      ...
      .select("id_curso, titulo, subtitulo, descripcion, precio, image_id, chat_url, duracion_label, nivel, categoria, featured, visible").maybeSingle();
